...
function toID(text) {
 ...
 class ModlogConverterSQLite {
	...
	constructor(
		databaseFile, textLogDir,
		isTesting, newestAllowedTimestamp
	) {;ModlogConverterSQLite.prototype.__init.call(this);
		...
		for (const {roomid} of roomids) {
			...
			for (const result of results) {
				...
				const entry = {
					...
					ip: result.ip,
					isGlobal: _optionalChain([result, 'access', _19 => _19.roomid, 'optionalAccess', _20 => _20.startsWith, 'call', _21 => _21('global-')]) || result.roomid === 'global' || result.is_global,
 ...
 class ModlogConverterTxt {
	...
	) {;ModlogConverterTxt.prototype.__init2.call(this);
		this.databaseFile = databaseFile;
		this.textLogDir = textLogDir;
		if (isTesting || Config.nofswriting) {
			this.isTesting = {
 ...
 const ModlogConverter = {
	async convert(
		...
		textLogDirectoryPath, outputLogPath, newestAllowedTimestamp,
	) {
