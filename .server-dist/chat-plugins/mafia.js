...
class Mafia extends Rooms.RoomGame {
	...
	unvote(userid, force = false) {
		...
		if (!player && this.dead[userid] && this.dead[userid].restless) player = this.dead[userid];
		if (!_optionalChain([player, 'optionalAccess', _6 => _6.voting])) return this.sendUser(userid, `|error|You are not voting for anyone.`);
	...
	voteBox() {
		...
		for (const [key, vote] of list) {
			buf += `${vote.count}${plur === key ? '*' : ''} ${_optionalChain([this, 'access', _7 => _7.playerTable, 'access', _8 => _8[key], 'optionalAccess', _9 => _9.safeName]) || 'No Vote'} (${vote.voters.map(a => _optionalChain([this, 'access', _10 => _10.playerTable, 'access', _11 => _11[a], 'optionalAccess', _12 => _12.safeName]) || a).join(', ')})<br />`;
		}
		return buf;
	}
	voteBoxFor(userid) {
