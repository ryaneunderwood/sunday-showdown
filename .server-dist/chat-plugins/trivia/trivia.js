 ...
 */
 async function requestAltMerge(from, to) {
	...
	if (!(await exports.database.getLeaderboardEntry(from, true))) {
		throw new Chat.ErrorMessage(`The user '${from}' does not have an entry in the Trivia leaderboard.`);
	}
	if (!(await exports.database.getLeaderboardEntry(to, true))) {
...
const triviaCommands = {
	...
	async new(target, room, user, connection, cmd) {
		...
		if (isRandomMode) {
			const recentHistory = await exports.database.getHistory(10);
	...
	async delete(target, room, user) {
		...
		await exports.database.ensureQuestionExists(question);
	...
	async qs(target, room, user) {
		...
		const list = await exports.database.getQuestions([category], Number.MAX_SAFE_INTEGER, {order: 'newestfirst'});
		if (!list.length) {
	...
	async rank(target, room, user) {
		...
		const allTimeScore = await exports.database.getLeaderboardEntry(userid, true);
		if (!allTimeScore) return this.sendReplyBox(this.tr`User '${name}' has not played any trivia games yet.`);
		const score = await exports.database.getLeaderboardEntry(userid, false) || {score: 0, totalPoints: 0, totalCorrectAnswers: 0};

		const ranks = await _asyncOptionalChain([(await exports.cachedLadder.get(false)), 'optionalAccess', async _19 => _19.ranks, 'access', async _20 => _20[userid]]);
		const allTimeRanks = await _asyncOptionalChain([(await exports.cachedLadder.get(true)), 'optionalAccess', async _21 => _21.ranks, 'access', async _22 => _22[userid]]);
		this.sendReplyBox(
	...
	async ladder(target, room, user, connection, cmd) {
		...
		const ladder = await _asyncOptionalChain([(await exports.cachedLadder.get(isAllTime)), 'optionalAccess', async _23 => _23.ladder]);
		if (!_optionalChain([ladder, 'optionalAccess', _24 => _24.length])) return this.errorReply(this.tr`No Trivia games have been played yet.`);
		...
		for (let i = Math.max(0, num - 100); i < num; i++) {
			...
			for (const leader of leaders) {
				const rank = await exports.database.getLeaderboardEntry(leader, isAllTime);
	...
	async removeleaderboardentry(target, room, user) {
		...
		if (!(await exports.database.getLeaderboardEntry(userid, true))) {
		...
		await Promise.all([
			exports.database.deleteLeaderboardEntry(userid, true),
			exports.database.deleteLeaderboardEntry(userid, false),
		]);
		exports.cachedLadder.invalidateCache();
