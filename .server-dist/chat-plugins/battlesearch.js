...
async function getBattleSearch(
 ...
 const pages = {
	async battlesearch(args, user, connection) {
		...
		const months = _lib.Utils.sortBy(
			(await _lib.FS.call(void 0, 'logs/').readdir()).filter(f => f.length === 7 && f.includes('-')),
			name => ({reverse: name})
		);
		if (!month) {
		...
		const tiers = _lib.Utils.sortBy(await _lib.FS.call(void 0, `logs/${month}/`).readdir(), tier => [
...
if (!exports.PM.isParentProcess) {
 ...
 //# sourceMappingURL=sourceMaps/battlesearch.js.map