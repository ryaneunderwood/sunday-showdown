...
function writeStats(line) {
 ...
 class HelpTicket extends Rooms.RoomGame {
	...
	close(result, staff) {
		...
		exports.tickets[this.ticket.userid] = this.ticket;
		writeTickets();
		this.room.modlog({action: 'TICKETCLOSE', isGlobal: false, loggedBy: _optionalChain([staff, 'optionalAccess', _6 => _6.id]) || 'unknown' });
		...
		if (!this.involvedStaff.size) {
			if (_optionalChain([staff, 'optionalAccess', _7 => _7.isStaff]) && staff.id !== this.ticket.userid) {
	...
	static getTextButton(ticket) {
		let buf = '';
		const titleBuf = [...ticket.text[0].split('\n').map(_lib.Utils.escapeHTML), ...ticket.text[1].split('<br />')].slice(0, 3);
		const noteBuf = Object.entries(ticket.notes || {})
			.map(([userid, note]) => _lib.Utils.html`${note} (by ${userid})`)
			.join('&#10;');
		const notes = ticket.notes ? `&#10;Staff notes:&#10;${noteBuf}` : '';
		const title = `title="${titleBuf.join('&#10;')}${notes}"`;
		const language = _optionalChain([Users, 'access', _8 => _8.get, 'call', _9 => _9(ticket.userid), 'optionalAccess', _10 => _10.language]) || '';
	...
	static notifyResolved(user, ticket, userid = user.id) {
		...
		if (_optionalChain([result, 'optionalAccess', _11 => _11.trim, 'call', _12 => _12()])) {
	...
	static getTypeId(name) {
		return _optionalChain([Object, 'access', _13 => _13.entries, 'call', _14 => _14(ticketTitles), 'access', _15 => _15.find, 'call', _16 => _16(entry => entry[1] === name), 'optionalAccess', _17 => _17[0]]) || toID(name);
	}
...
function notifyUnclaimedTicket(hasAssistRequest) {
 ...
 function notifyStaff() {
	...
	for (const ticket of sortedTickets) {
		...
		const ticketRoom = Rooms.get(`help-${ticket.userid}`);
		const ticketGame = _optionalChain([ticketRoom, 'optionalAccess', _18 => _18.getGame, 'call', _19 => _19(HelpTicket)]);
		if (!ticket.claimed) {
			hasUnclaimed = true;
	...
	for (const user of Object.values(room.users)) {
		...
		for (const connection of user.connections) {
			if (_optionalChain([connection, 'access', _20 => _20.openPages, 'optionalAccess', _21 => _21.has, 'call', _22 => _22('help-tickets')])) {
...
function checkIp(ip) {
 ...
 async function getOpponent(link, submitter) {
	...
	if (_optionalChain([room, 'optionalAccess', _23 => _23.battle])) {
 ...
 async function getBattleLog(battle) {
	...
	try {
		...
		if (_optionalChain([data, 'access', _24 => _24.log, 'optionalAccess', _25 => _25.length])) {
			return {
...
const cheatingScenarios = [
 ...
 const textTickets = {
	...
	inapname: {
		...
		getReviewDisplay(ticket, staff, conn, state) {
			...
			const name = _lib.Utils.escapeHTML(_optionalChain([Users, 'access', _26 => _26.getExact, 'call', _27 => _27(tar), 'optionalAccess', _28 => _28.name]) || tar);
			...
			buf += `<details ${_optionalChain([state, 'optionalAccess', _29 => _29.list]) ? 'open' : ''} class="readmore">`;
		...
		onSubmit(ticket, text) {
			if (!_optionalChain([ticket, 'access', _30 => _30.meta, 'optionalAccess', _31 => _31.startsWith, 'call', _32 => _32('user-')])) {
	...
	inappokemon: {
		...
		async getReviewDisplay(ticket, staff, conn) {
			...
			const battleRooms = links.map(r => Rooms.get(r)).filter(room => _optionalChain([room, 'optionalAccess', _33 => _33.battle])) ;
			...
			for (const [i, ip] of ips.entries()) {
				...
				if (ipPunishments) {
					const str = ipPunishments.map(p => (
						`${_optionalChain([Punishments, 'access', _34 => _34.punishmentTypes, 'access', _35 => _35.get, 'call', _36 => _36(p.type), 'optionalAccess', _37 => _37.desc]) || p.type} as ${p.id}${p.reason ? ` (${p.reason})` : ''}`
					));
 ...
 const pages = {
	help: {
		request(query, user, connection) {
			...
			if (_optionalChain([ticket, 'optionalAccess', _38 => _38.open]) || ipTicket) {
		...
		tickets(query, user, connection) {
			...
			for (const ticket of sortedTickets) {
				...
				const ticketRoom = Rooms.get(`help-${ticket.userid}`);
				buf += `<tr><td>${icon}</td>`;
				const language = Chat.languages.get(_optionalChain([ticketRoom, 'optionalAccess', _39 => _39.settings, 'access', _40 => _40.language]) || '');
 ...
 const commands = {
	report(target, room, user) {
		...
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _41 => _41.battle])) return this.errorReply(this.tr`This command cannot be broadcast in battles.`);
	...
	appeal(target, room, user) {
		...
		const meta = this.pmTarget ? `-user-${this.pmTarget.id}` : this.room ? `-room-${this.room.roomid}` : '';
		if (this.broadcasting) {
			if (_optionalChain([room, 'optionalAccess', _42 => _42.battle])) return this.errorReply(this.tr`This command cannot be broadcast in battles.`);
	...
	helpticket: {
		...
		async submit(target, room, user, connection, cmd) {
			...
			let ticket = exports.tickets[user.id];
			const ipTicket = checkIp(user.latestIp);
			if (_optionalChain([ticket, 'optionalAccess', _43 => _43.open]) || ipTicket) {
				if (!ticket && ipTicket) ticket = ipTicket;
				if (ticket.text) {
			...
			if (textTicket) {
				...
				const validation = await _optionalChain([textTicket, 'access', _44 => _44.checker, 'optionalCall', _45 => _45(text, contextString || '', ticket.type, user, reportTarget)]);
				...
				writeTickets();
				notifyStaff();
				_optionalChain([textTicket, 'access', _46 => _46.onSubmit, 'optionalCall', _47 => _47(ticket, [text, contextString], this.user, this.connection)]);
		...
		close(target, room, user) {
			...
			if (!_optionalChain([ticket, 'optionalAccess', _48 => _48.open]) || (ticket.userid !== user.id && !user.can('lock'))) {
		...
		async ban(target, room, user) {
			...
			for (const userObj of affected) {
				...
				if (_optionalChain([targetTicket, 'optionalAccess', _49 => _49.open])) targetTicket.open = false;
				const helpRoom = Rooms.get(`help-${userObjID}`);
				if (helpRoom) {
					const ticketGame = helpRoom.getGame(HelpTicket);
					ticketGame.writeStats('ticketban');
					helpRoom.destroy();
				} else if (_optionalChain([targetTicket, 'optionalAccess', _50 => _50.text])) {
					await HelpTicket.modlog({
						action: `TICKETBAN`,
						loggedBy: user.id,
					...
					if (staffRoom) {
						for (const curUser of Object.values(staffRoom.users)) {
							for (const conn of curUser.connections) {
								if (_optionalChain([conn, 'access', _51 => _51.openPages, 'optionalAccess', _52 => _52.has, 'call', _53 => _53(`help-text-${userObjID}`)])) {
		...
		unban(target, room, user) {
			...
			this.checkCan('lock');
			target = toID(target);
			const targetID = _optionalChain([Users, 'access', _54 => _54.get, 'call', _55 => _55(target), 'optionalAccess', _56 => _56.id]) || target ;
			...
			_optionalChain([Users, 'access', _57 => _57.get, 'call', _58 => _58(target), 'optionalAccess', _59 => _59.popup, 'call', _60 => _60(`${user.name} has ticket unbanned you.`)]);
		},
 ...
 const punishmentfilter = (user, punishment) => {
	...
	if (typeof user === 'object') {
		...
		if (_optionalChain([helpRoom, 'optionalAccess', _61 => _61.game, 'optionalAccess', _62 => _62.gameid]) !== 'helpticket') return;
 ...
 const loginfilter = (user) => {
	...
	if (_optionalChain([ticket, 'optionalAccess', _63 => _63.resolved])) {
 ...
 const handlers = {
	onRoomClose(room, user, conn, isPage) {
		...
		if (_optionalChain([ticket, 'optionalAccess', _64 => _64.open]) && ticket.claimed === user.id) {
			ticket.claimed = null;
			if (_optionalChain([ticket, 'access', _65 => _65.state, 'optionalAccess', _66 => _66.claimTime])) {
