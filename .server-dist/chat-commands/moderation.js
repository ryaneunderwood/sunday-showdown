...
Punishments.addPunishmentType('YEARLOCK', "Locked for a year", (user, punishment) => {
 ...
 const commands = {
	...
	warn(target, room, user) {
		...
		this.checkChat();
		if (_optionalChain([room, 'optionalAccess', _33 => _33.settings, 'access', _34 => _34.isPersonal]) && !user.can('warn' )) {
		...
		const saveReplay = globalWarn && _optionalChain([room, 'optionalAccess', _35 => _35.battle]);
		if (!_optionalChain([targetUser, 'optionalAccess', _36 => _36.connected])) {
	...
	unmute(target, room, user) {
		...
		const successfullyUnmuted = room.unmute(
			_optionalChain([targetUser, 'optionalAccess', _38 => _38.id]) || toID(targetUsername), `Your mute in '${room.title}' has been lifted.`
		);
	...
	async lock(target, room, user, connection, cmd) {
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _39 => _39.battle])) this.parse('/savereplay forpunishment');
		return true;
	},
	lockhelp: [
	...
	unlock(target, room, user) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _40 => _40.namelocked])) {
			return this.errorReply(`User ${targetUser.name} is namelocked, not locked. Use /unnamelock to unnamelock them.`);
		}
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _41 => _41.locked]) && targetUser.locked.startsWith('#')) {
	...
	unlockip(target, room, user) {
		...
		for (const curUser of Users.findUsers([], [target])) {
			if (
				(range ? curUser.locked === '#rangelock' : !_optionalChain([curUser, 'access', _42 => _42.locked, 'optionalAccess', _43 => _43.startsWith, 'call', _44 => _44('#')])) &&
	...
	async globalban(target, room, user, connection, cmd) {
		...
		if (Users.isTrusted(userid)) {
			if (force) {
				const from = runCrisisDemote(userid);
				Monitor.log(`[CrisisMonitor] ${name} was globally banned by ${user.name} and demoted from ${_optionalChain([from, 'optionalAccess', _45 => _45.join, 'call', _46 => _46(", ")])}.`);
				this.globalModlog("CRISISDEMOTE", targetUser, ` from ${_optionalChain([from, 'optionalAccess', _47 => _47.join, 'call', _48 => _48(", ")])}`);
			} else {
		...
		const {privateReason, publicReason} = this.parseSpoiler(reason);
		_optionalChain([targetUser, 'optionalAccess', _49 => _49.popup, 'call', _50 => _50(
		...
		_optionalChain([room, 'optionalAccess', _51 => _51.hideText, 'call', _52 => _52([
			...affected.map(u => u.id),
			toID(inputUsername),
		])]);
	...
	banip(target, room, user) {
		...
		if (_optionalChain([curPunishment, 'optionalAccess', _53 => _53.type]) === 'BAN') {
	...
	yearlockname(target, room, user) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _54 => _54.registered]) && !force) {
	...
	modnote(target, room, user, connection) {
		...
		const targetUserid = toID(_optionalChain([/\[([^\]]+)\]/, 'access', _55 => _55.exec, 'call', _56 => _56(target), 'optionalAccess', _57 => _57[1]])) || null;
		if (
	...
	promote(target, room, user, connection, cmd) {
		...
		const userid = toID(targetUsername);
		const name = _optionalChain([targetUser, 'optionalAccess', _58 => _58.name]) || targetUsername;

		if (!userid) return this.parse('/help promote');

		const currentGroup = _optionalChain([targetUser, 'optionalAccess', _59 => _59.tempGroup]) || Users.globalAuth.get(userid);
	...
	trustuser(target, room, user, connection, cmd) {
		...
		if (rest) return this.errorReply(`This command does not support specifying a reason.`);
		const userid = toID(targetUsername);
		const name = _optionalChain([targetUser, 'optionalAccess', _60 => _60.name]) || targetUsername;
	...
	sectionleader(target, room, user, connection, cmd) {
		...
		if (Users.globalAuth.sectionLeaders.has(_optionalChain([targetUser, 'optionalAccess', _61 => _61.id]) || userid) && !demoting) {
			throw new Chat.ErrorMessage(`${name} is already a Section Leader of ${_roomsettings.RoomSections.sectionNames[section]}.`);
		} else if (!Users.globalAuth.sectionLeaders.has(_optionalChain([targetUser, 'optionalAccess', _62 => _62.id]) || userid) && demoting) {
		...
		if (!demoting) {
			...
			_optionalChain([targetUser, 'optionalAccess', _63 => _63.popup, 'call', _64 => _64(`You were appointed Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`)]);
		} else {
			...
			if (_optionalChain([staffRoom, 'optionalAccess', _65 => _65.auth, 'access', _66 => _66.getDirect, 'call', _67 => _67(userid)])  === '\u25B8') this.parse(`/msgroom staff,/roomdeauth ${userid}`);
			_optionalChain([targetUser, 'optionalAccess', _68 => _68.popup, 'call', _69 => _69(`You were demoted from Section Leader of ${_roomsettings.RoomSections.sectionNames[section]} by ${user.name}.`)]);
		}
	...
	htmldeclare(target, room, user) {
		...
		for (const u in room.users) {
			_optionalChain([Users, 'access', _70 => _70.get, 'call', _71 => _71(u), 'optionalAccess', _72 => _72.sendTo, 'call', _73 => _73(
				room,
	...
	notifyuser(target, room, user, connection, cmd) {
		...
		if (!_optionalChain([targetUser, 'optionalAccess', _74 => _74.connected])) return this.errorReply(`User '${targetUsername}' not found.`);
	...
	forcerename(target, room, user) {
		...
		const targetID = _optionalChain([targetUser, 'optionalAccess', _75 => _75.id]) || toID(targetUsername);
		...
		if (_optionalChain([targetUser, 'optionalAccess', _76 => _76.connected])) {
		...
		if (_optionalChain([room, 'optionalAccess', _77 => _77.roomid]) !== 'staff') {
			if (_optionalChain([room, 'optionalAccess', _78 => _78.roomid, 'access', _79 => _79.startsWith, 'call', _80 => _80('help-')])) {
				this.addModAction(`${_optionalChain([targetUser, 'optionalAccess', _81 => _81.name]) || targetID} ${forceRenameMessage}`);
			} else {
				this.privateModAction(`${_optionalChain([targetUser, 'optionalAccess', _82 => _82.name]) || targetID} ${forceRenameMessage}`);
			}
		}
		const roomMessage = this.pmTarget ? `<PM:${this.pmTarget.id}>` :
			room && room.roomid !== 'staff' ? `«<a href="/${room.roomid}" target="_blank">${room.roomid}</a>» ` :
			'';
		const rankMessage = _optionalChain([targetUser, 'optionalAccess', _83 => _83.getAccountStatusString, 'call', _84 => _84()]) || "";
		Rooms.global.notifyRooms(
			['staff'],
			`|html|${roomMessage}` + _lib.Utils.html`<span class="username">${_optionalChain([targetUser, 'optionalAccess', _85 => _85.name]) || targetID}</span> ${rankMessage} ${forceRenameMessage}`
		);

		_optionalChain([targetUser, 'optionalAccess', _86 => _86.resetName, 'call', _87 => _87(true)]);
	...
	async namelock(target, room, user, connection, cmd) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _88 => _88.namelocked]) && !week) {
			return this.errorReply(`User '${targetUser.name}' is already namelocked.`);
		}
		const {privateReason, publicReason} = this.parseSpoiler(reason);
		const reasonText = publicReason ? ` (${publicReason})` : `.`;
		this.privateGlobalModAction(`${_optionalChain([targetUser, 'optionalAccess', _89 => _89.name]) || userid} was ${week ? 'week' : ''}namelocked by ${user.name}${reasonText}`);
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _90 => _90.battle])) this.parse('/savereplay forpunishment');
	...
	unnamelock(target, room, user) {
		...
		const targetUser = Users.get(target);
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _91 => _91.namelocked])) {
	...
	async groupchatban(target, room, user, connection, cmd) {
		...
		for (const roomid of createdGroupchats) {
			...
			const participants = _optionalChain([targetRoom, 'access', _92 => _92.warnParticipants, 'optionalCall', _93 => _93(
