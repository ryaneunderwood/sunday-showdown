...
for (const avatar of avatarTableGnomowladny) avatarTable.add(avatar);
 ...
 const commands = {
	...
	help(target, room, user) {
		...
		const requiredPerm = _optionalChain([curHandler, 'optionalAccess', _22 => _22.requiredPermission]) || 'lock';
		if (_optionalChain([curHandler, 'optionalAccess', _23 => _23.isPrivate]) && !user.can(requiredPerm )) {
			return this.errorReply(this.tr`The command '/${target}' does not exist.`);
		}
