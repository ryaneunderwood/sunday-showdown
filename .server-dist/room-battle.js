...
const TIMER_COOLDOWN = 20 * SECONDS;
 ...
 class RoomBattle extends RoomGames.RoomGame {
	...
	async onEnd(winner) {
		...
		Chat.runHandlers('BattleEnd', this, winnerid, [p1id, p2id, _optionalChain([this, 'access', _19 => _19.p3, 'optionalAccess', _20 => _20.id]), _optionalChain([this, 'access', _21 => _21.p4, 'optionalAccess', _22 => _22.id])].filter(Boolean));
		if (this.room.rated) {
			this.room.rated = 0;
		...
		if (this.replaySaved || Config.autosavereplays) {
			...
			if (_optionalChain([uploader, 'optionalAccess', _23 => _23.connections, 'access', _24 => _24[0]])) {
		...
		if (_optionalChain([parentGame, 'optionalAccess', _25 => _25.onBattleWin])) {
			// @ts-ignore
	...
	onLeave(user, oldUserid) {
		...
		if (_optionalChain([player, 'optionalAccess', _26 => _26.active])) {
	...
	addPlayer(user, playerOpts) {
		...
		if (_optionalChain([user, 'optionalAccess', _27 => _27.inRooms, 'access', _28 => _28.has, 'call', _29 => _29(this.roomid)])) this.onConnect(user);
		return player;
	}
	...
	checkPrivacySettings(options) {
		...
		if (privacySetter.size) {
			...
			} else if (!options.tour || (_optionalChain([room, 'access', _30 => _30.tour, 'optionalAccess', _31 => _31.allowModjoin]))) {
	...
	checkForcedUserSettings(user) {
		...
		if (
			this.players.some(p => _optionalChain([p, 'access', _32 => _32.getUser, 'call', _33 => _33(), 'optionalAccess', _34 => _34.battleSettings, 'access', _35 => _35.special])) ||
	...
	static battleForcedSetting(user, key) {
		...
		if (!_optionalChain([Config, 'access', _36 => _36.forcedprefixes, 'optionalAccess', _37 => _37[key]])) return null;
	...
	updatePlayer(player, user, playerOpts) {
		...
		if (user) {
			const options = {
				name: player.name,
				avatar: user.avatar,
				team: _optionalChain([playerOpts, 'optionalAccess', _38 => _38.team]),
			};
			void this.stream.write(`>player ${slot} ` + JSON.stringify(options));
	...
	start() {
		...
		const suspectTest = _optionalChain([Chat, 'access', _39 => _39.plugins, 'access', _40 => _40['suspect-tests'], 'optionalAccess', _41 => _41.suspectTests, 'access', _42 => _42[this.format]]);
		if (suspectTest) {
		...
		try {
			...
			if (battle) {
				for (const side of battle.sides) {
					if (_optionalChain([side, 'optionalAccess', _43 => _43.requestState])) {
