 ...
 */
 ...
 const Chat = new (_class = class {
	 ...
	 */
	__init21() {this.database = _lib.SQL.call(void 0, module, {file: ('Config' in global && Config.nofswriting) ? ':memory:' : PLUGIN_DATABASE_PATH})}
	...
	async prepareDatabase() {
		...
		for (const migrationFile of (await _lib.FS.call(void 0, migrationsFolder).readdir())) {
			const migrationVersion = parseInt(_optionalChain([/v(\d+)\.sql$/, 'access', _68 => _68.exec, 'call', _69 => _69(migrationFile), 'optionalAccess', _70 => _70[1]]) || '');
		...
		exports.Chat.destroyHandlers.push(() => _optionalChain([exports.Chat, 'access', _71 => _71.database, 'optionalAccess', _72 => _72.destroy, 'call', _73 => _73()]));
	}
	...
	parse(message, room, user, connection) {
		...
		const initialRoomlogLength = _optionalChain([room, 'optionalAccess', _74 => _74.log, 'access', _75 => _75.getLineCount, 'call', _76 => _76()]);
		const context = new CommandContext({message, room, user, connection});
		const start = Date.now();
		const result = context.parse();
		if (typeof _optionalChain([result, 'optionalAccess', _77 => _77.then]) === 'function') {
			void result.then(() => {
	...
	logSlowMessage(start, context) {
		...
		const logMessage = (
			...
			`<${context.room ? context.room.roomid : context.pmTarget ? `PM:${_optionalChain([context, 'access', _78 => _78.pmTarget, 'optionalAccess', _79 => _79.name])}` : 'CMD'}> ` +
	...
	loadPlugin(file) {
		...
		this.loadPluginData(plugin, _optionalChain([file, 'access', _80 => _80.split, 'call', _81 => _81('/'), 'access', _82 => _82.pop, 'call', _83 => _83(), 'optionalAccess', _84 => _84.slice, 'call', _85 => _85(0, -3)]) || file);
	}
	annotateCommands(commandTable, namespace = '') {
		for (const cmd in commandTable) {
			...
			entry.requiresRoom = _optionalChain([/requireRoom\((?:'|"|`)(.*?)(?:'|"|`)/, 'access', _86 => _86.exec, 'call', _87 => _87(handlerCode), 'optionalAccess', _88 => _88[1]])  || /this\.requireRoom\(/.test(handlerCode);
			...
			entry.requiredPermission = _optionalChain([/this\.(?:checkCan|privately(?:Check)?Can)\(['`"]([a-zA-Z0-9]+)['"`](\)|, )/, 'access', _89 => _89.exec, 'call', _90 => _90(handlerCode), 'optionalAccess', _91 => _91[1]]);
...

 {
	...
	toDurationString(val, options = {}) {
		...
		let precision = (_optionalChain([options, 'optionalAccess', _92 => _92.precision]) ? options.precision : 3);
		if (_optionalChain([options, 'optionalAccess', _93 => _93.hhmmss])) {
	...
	refreshPageFor(
		...
		for (const id in room.users) {
			if (_optionalChain([ignoreUsers, 'optionalAccess', _94 => _94.includes, 'call', _95 => _95(id )])) continue;
	...
	maybeNotifyBlocked(blocked, targetUser, user) {
	 ...
	 __init29() {this.formatText = _chatformatter.formatText}
	 __init30() {this.linkRegex = _chatformatter.linkRegex}
	 __init31() {this.stripFormatting = _chatformatter.stripFormatting}

	 __init32() {this.filterWords = {}}
	 __init33() {this.monitors = {}}
...
(CommandContext.prototype ).targetUserOrSelf = function ( target, exactName) {
	...
	this.targetUsername = _optionalChain([user, 'optionalAccess', _96 => _96.name]) || target;
	return user;
};
